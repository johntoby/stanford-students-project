...
kubectl exec -n vault-system deployment/vault -- vault auth -method=token token=root
kubectl exec -n vault-system deployment/vault -- vault kv put secret/database username=postgres password=postgres
